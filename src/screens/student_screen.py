...
from src.database.db import get_all_students,get_student_subjects,get_student_attendance,unenroll_student_to_subject
    ...
    for i,sub_node in enumerate(subjects):
        ...
        def unenroll_btn():
            if st.button("Unenroll from this course",type='tertiary',width='stretch'):
                ...
                st.rerun()
        with cols[i%2]:
            subject_card( name=sub['name'], code=sub['subject_code'],section=sub['section'],
                stats=[
                    ('label','Total',stats['total']),
                    ('label','Attended',stats['attended'])
                ],
